...
if __name__=="__main__":
  ...
  # 生成深度可视化图
  depth_vis = disp.copy()
  depth_vis[np.isinf(depth_vis)] = 0  # 避免 inf 干扰
  depth_vis = (depth_vis - depth_vis.min()) / (depth_vis.max() - depth_vis.min() + 1e-8)  # 归一化到0-1
  depth_vis = (depth_vis * 255).astype('uint8')
  #depth_vis_color = cv2.applyColorMap(depth_vis, cv2.COLORMAP_JET)  # 彩色映射
  depth_vis = np.stack([depth_vis]*3, axis=-1)  # 灰度转3通道

  # 拼接左图和深度图
  #vis = np.concatenate([img0_ori, depth_vis_color], axis=1)
  vis = np.concatenate([img0_ori, depth_vis], axis=1)
  ...
  if args.get_pc:
    ...
    xyz_map = depth2xyzmap(depth, K)
        # 修正点云坐标系：前后、左右、上下全部取反
    xyz_map[..., 1] = -xyz_map[..., 1]  # y轴取反（上下）
    xyz_map[..., 2] = -xyz_map[..., 2]  # z轴取反（前后）
